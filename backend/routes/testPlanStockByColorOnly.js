...
import { planStockUpdateByColorOnly } from "../tools/woocommerce.js";
...
router.post("/", async (req, res) => {
  try {
    ...
    const result = await planStockUpdateByColorOnly({
      baseUrl,
      consumerKey,
      consumerSecret,
      productSearch,
    ...
    console.error(
      "Error en /test-plan-stock-by-color-only:",
      error?.response?.data || error.message
    );

    res.status(500).json({
      error: "Error al preparar plan de stock por color",
