...
function userHasAdminAccess(data: any) {
...
export default function ChatWindow() {
  ...
  const [userMe, setUserMe] = useState<{ usa_precio_efectivo?: boolean; role?: string; user_role?: string; is_admin?: boolean; isAdmin?: boolean; admin?: boolean } | null>(null);
...
const [moveTargetProduct, setMoveTargetProduct] = useState<EditFoundProduct | null>(null);
  ...
  const isCurrentUserAdmin = useMemo(() => {
    if (userHasAdminAccess(userMe)) return true;
    if (typeof window === "undefined") return false;

    try {
      const rawUser = localStorage.getItem("user");
      if (!rawUser) return false;
      return userHasAdminAccess(JSON.parse(rawUser));
    } catch {
      return false;
    }
  }, [userMe]);
...
async function openUsersPanel() {
  if (!isCurrentUserAdmin) {
    pushAssistantInfo("Esta opción solo está disponible para el usuario admin.");
    return;
  }
...
onMouseLeave={(e) => {
  ...
  el.style.background = "linear-gradient(180deg, #111827 0%, #0f172a 100%)";
                  ...
                  >
      {activeAction === "users" && isCurrentUserAdmin ? (
        <div
          style={{
            margin: "16px",
            padding: "16px",
